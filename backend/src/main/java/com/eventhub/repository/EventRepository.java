    ...
    @Query(value = """
            ...
            and (
                ...
                or (:status = 'OPEN' and e.startsAt > :now
                    and (select coalesce(sum(r1.quantity), 0) from Registration r1 where r1.event.id = e.id) < e.capacity)
                or (:status = 'FULL' and e.startsAt > :now
                    and (select coalesce(sum(r2.quantity), 0) from Registration r2 where r2.event.id = e.id) >= e.capacity)
                or (:status = 'ENDED' and e.startsAt <= :now)
            ...
            """)
    Page<EventListRow> search(@Param("userId") Long userId, @Param("search") String search, @Param("category") String category,
                              @Param("status") String status, @Param("now") LocalDateTime now,
                              Pageable pageable);

    @Query("select count(e.id) from Event e where e.startsAt > :now and " +
            "(select coalesce(sum(r.quantity), 0) from Registration r where r.event.id = e.id) < e.capacity")
