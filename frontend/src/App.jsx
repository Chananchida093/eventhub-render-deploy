...
function DetailImageEditor({ images, onChange, locale }) {
  ...
  return <section className="detail-image-editor"><div><strong>{locale === 'th' ? 'รูปประกอบในรายละเอียด' : 'Detail content images'}</strong><small>{locale === 'th' ? 'เพิ่มได้หลายรูป และเลือกตำแหน่งที่ต้องการแสดงในหน้าอีเวนต์' : 'Add multiple images and choose where each appears on the event page.'}</small></div>{images.map((image, index) => <div className="detail-image-row" key={index}><img src={image.url || '/art-design.svg'} alt="" /><input value={image.url} onChange={e => change(index, 'url', e.target.value)} placeholder="https://…" /><select value={image.placement} onChange={e => change(index, 'placement', e.target.value)}><option value="TOP">{locale === 'th' ? 'ก่อนรายละเอียด' : 'Before details'}</option><option value="DETAILS">{locale === 'th' ? 'ในรายละเอียด' : 'In details'}</option><option value="BOTTOM">{locale === 'th' ? 'ท้ายรายละเอียด' : 'After details'}</option></select><button type="button" onClick={() => onChange(images.filter((_, imageIndex) => imageIndex !== index))} aria-label="Remove image"><Trash2 size={15} /></button></div>)}<button type="button" className="button button-plain button-small" onClick={add}><Plus size={15} /> {locale === 'th' ? 'เพิ่มรูปประกอบ' : 'Add detail image'}</button></section>
}
...
function AdminEvents({ page, params, loading, onParamsChange, refresh, setModal, setToast, locale }) {
  ...
  return <section className="container page-section admin-page">
    ...
    <section className="analytics-board"><div><span>{locale === 'th' ? 'การเข้าชมเว็บไซต์' : 'Site views'}</span><strong>{analytics?.siteViews ?? '—'}</strong><small>{locale === 'th' ? `${analytics?.uniqueVisitors ?? 0} ผู้เข้าชมไม่ซ้ำ` : `${analytics?.uniqueVisitors ?? 0} unique visitors`}</small></div><div><span>{locale === 'th' ? 'การดูรายละเอียดอีเวนต์' : 'Event detail views'}</span><strong>{analytics?.eventViews ?? '—'}</strong><small>{locale === 'th' ? 'นับจากการเปิดหน้ารายละเอียด' : 'Recorded when details are opened'}</small></div><div><span>{locale === 'th' ? 'อัตราการจองต่อการดู' : 'View-to-booking signal'}</span><strong>{analytics?.eventViews ? `${Math.round((analytics.registrations / analytics.eventViews) * 100)}%` : '—'}</strong><small>{locale === 'th' ? 'ใช้ดูความสนใจของผู้เข้าชม' : 'Interest signal from real activity'}</small></div></section>
