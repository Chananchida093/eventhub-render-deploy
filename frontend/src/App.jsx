  ...
  return <div className="app-shell">
    <header className="site-header">
      ...
      <div className="header-actions">
        ...
        </div> : <div className="auth-actions"><button className="button button-plain button-small" onClick={() => setModal({ type: 'signup' })}>{locale === 'th' ? 'สมัครสมาชิก' : 'Create account'}</button><button className="button button-dark button-small" onClick={() => setModal({ type: 'login' })}>{t(locale, 'signIn')}</button></div>}
      ...
      {menuOpen && <nav className="mobile-nav" aria-label={t(locale, 'discover')}>
        ...
        {!user && <><button onClick={() => { setModal({ type: 'signup' }); setMenuOpen(false) }}>{locale === 'th' ? 'สมัครสมาชิก' : 'Create account'}</button><button onClick={() => { setModal({ type: 'login' }); setMenuOpen(false) }}>{t(locale, 'signIn')}</button></>}
    ...
    {modal?.type === 'signup' && <SignupModal setModal={setModal} setUser={setUser} setView={setView} refresh={loadEvents} setToast={setToast} locale={locale} />}
...
function SignupModal({ setUser, setView, setModal, refresh, setToast, locale }) {
