...
function EventEditor({ event, onClose, onSaved, locale }) {
  const initial = event ? { ...event, imageUrl: event.imageUrl || '', detailImageUrl: event.detailImageUrl || '', startsAt: event.startsAt.slice(0,16), ticketTypes: ticketTypesFor(event).map(ticket => ({ ...ticket, price: String(ticket.price) })) } : { title:'', description:'', location:'', startsAt:'', capacity:30, category:'COMMUNITY', imageUrl:'/art-design.svg', detailImageUrl:'', ticketTypes:[{ name:'General admission', description:'', price:'0', capacity:30 }] }
