...
const translations = {
  en: {
    discover: 'Discover', events: 'Events', registrations: 'My registrations', manage: 'Manage events', admin: 'Admin', signIn: 'Log in', signUp: 'Sign up', signOut: 'Sign out', member: 'Member', administrator: 'Administrator',
    heroEyebrow: 'Find your next reason to gather', heroTitle: 'Big ideas start<br />when people show up.', heroBody: 'Discover workshops, talks, and community moments curated for curious people.', browse: 'Explore all events', upcoming: 'Upcoming events', popularThisWeek: 'Popular events this week', eventCount: ({ count }) => `${count} ${count === 1 ? 'event' : 'events'}`, search: 'Search events, places, or topics', all: 'All events', available: 'Available', registered: 'Registered',
    ...
    newEvent: 'New event', editEvent: 'Edit event', eventTitle: 'Event title', description: 'Description', location: 'Location', dateTime: 'Date & time', maximum: 'Maximum participants', cancelAction: 'Cancel', saveEvent: 'Save event', saveChanges: 'Save changes', saving: 'Saving…', temporaryTitle: 'e.g. Product Design Workshop', descriptionHint: 'What will attendees learn or experience?', locationHint: 'Building and room', registrationList: 'Registration list', spotsFilled: '{{count}} / {{capacity}} registered', registeredOf: '{{count}} / {{capacity}} registered', spotsRemaining: '{{count}} spots left', attendeeName: 'Name', registeredAt: 'Registered at', noAttendees: 'No registrations yet', attendeesBody: 'Attendees will appear here after they register.',
    memberAccess: 'Member access', signInTitle: 'Log in to save your spot', signInBody: 'Your registrations stay tied to your account, so your schedule is always up to date.', email: 'Email', password: 'Password', signingIn: 'Signing in…', demoAccounts: 'Demo accounts', userDemo: 'User: user@event.local / password', adminDemo: 'Admin: admin@event.local / admin123', createAccount: 'Create your account', createAccountBody: 'Join Gather to save events, reserve a seat, and keep your schedule in one place.', displayName: 'Display name', confirmPassword: 'Confirm password', passwordHint: 'At least 8 characters', signingUp: 'Creating account…', newToGather: 'New to Gather?', hasAccount: 'Already have an account?', switchToLogin: 'Log in', switchToSignup: 'Create an account', accountCreated: 'Account created', passwordMismatch: 'Passwords do not match.', accountExists: 'An account with this email already exists.', adminOnly: 'Admin access is required to manage events.',
  ...
  th: {
    discover: 'ค้นหาอีเวนต์', events: 'อีเวนต์', registrations: 'การลงทะเบียนของฉัน', manage: 'จัดการอีเวนต์', admin: 'Admin', signIn: 'เข้าสู่ระบบ', signUp: 'สมัครสมาชิก', signOut: 'ออกจากระบบ', member: 'สมาชิก', administrator: 'ผู้ดูแลระบบ',
    heroEyebrow: 'เหตุผลดี ๆ ที่จะได้มาเจอกัน', heroTitle: 'ไอเดียที่ดี<br />เริ่มต้นเมื่อเราออกมาเจอกัน', heroBody: 'ค้นหาเวิร์กช็อป ทอล์ก และกิจกรรมชุมชนที่คัดสรรมาเพื่อคนช่างสงสัย', browse: 'ดูอีเวนต์ทั้งหมด', upcoming: 'อีเวนต์ที่กำลังจะมาถึง', popularThisWeek: 'อีเวนต์น่าสนใจประจำสัปดาห์', eventCount: ({ count }) => `${count} อีเวนต์`, search: 'ค้นหาชื่อ สถานที่ หรือหัวข้อ', all: 'อีเวนต์ทั้งหมด', available: 'ยังมีที่ว่าง', registered: 'ลงทะเบียนแล้ว',
    ...
    newEvent: 'อีเวนต์ใหม่', editEvent: 'แก้ไขอีเวนต์', eventTitle: 'ชื่ออีเวนต์', description: 'รายละเอียด', location: 'สถานที่', dateTime: 'วันและเวลา', maximum: 'จำนวนผู้เข้าร่วมสูงสุด', cancelAction: 'ยกเลิก', saveEvent: 'บันทึกอีเวนต์', saveChanges: 'บันทึกการแก้ไข', saving: 'กำลังบันทึก…', temporaryTitle: 'เช่น เวิร์กช็อปออกแบบผลิตภัณฑ์', descriptionHint: 'ผู้เข้าร่วมจะได้เรียนรู้หรือสัมผัสอะไรบ้าง', locationHint: 'อาคารและห้อง', registrationList: 'รายชื่อผู้ลงทะเบียน', spotsFilled: 'ลงทะเบียนแล้ว {{count}} / {{capacity}} คน', registeredOf: 'ลงทะเบียนแล้ว {{count}} / {{capacity}} คน', spotsRemaining: 'เหลือ {{count}} ที่นั่ง', attendeeName: 'ชื่อ', registeredAt: 'เวลาลงทะเบียน', noAttendees: 'ยังไม่มีผู้ลงทะเบียน', attendeesBody: 'รายชื่อผู้เข้าร่วมจะแสดงที่นี่เมื่อมีการลงทะเบียน',
    memberAccess: 'สำหรับสมาชิก', signInTitle: 'เข้าสู่ระบบเพื่อจองที่นั่ง', signInBody: 'รายการลงทะเบียนจะผูกกับบัญชีของคุณ เพื่อให้ตารางอีเวนต์อัปเดตเสมอ', email: 'อีเมล', password: 'รหัสผ่าน', signingIn: 'กำลังเข้าสู่ระบบ…', demoAccounts: 'บัญชีสำหรับทดลองใช้', userDemo: 'ผู้ใช้: user@event.local / password', adminDemo: 'ผู้ดูแล: admin@event.local / admin123', createAccount: 'สร้างบัญชีใหม่', createAccountBody: 'เข้าร่วม Gather เพื่อบันทึกอีเวนต์ จองที่นั่ง และจัดตารางของคุณไว้ในที่เดียว', displayName: 'ชื่อที่แสดง', confirmPassword: 'ยืนยันรหัสผ่าน', passwordHint: 'อย่างน้อย 8 ตัวอักษร', signingUp: 'กำลังสร้างบัญชี…', newToGather: 'ยังไม่มีบัญชี Gather?', hasAccount: 'มีบัญชีอยู่แล้ว?', switchToLogin: 'เข้าสู่ระบบ', switchToSignup: 'สร้างบัญชีใหม่', accountCreated: 'สร้างบัญชีเรียบร้อยแล้ว', passwordMismatch: 'รหัสผ่านไม่ตรงกัน', accountExists: 'อีเมลนี้มีบัญชีอยู่แล้ว', adminOnly: 'ต้องเข้าสู่ระบบด้วยบัญชี Admin เพื่อจัดการอีเวนต์',
...
function formatDateTime(value, locale) { return `${formatDate(value, locale)} · ${formatTime(value, locale)}` }
...
function App() {
  ...
  async function handleAuthenticated(current, requestedEvent) {
    setUser(current)
    if (requestedEvent) {
      try {
        await api.register(requestedEvent.id)
        setSelectedEvent(prev => prev?.id === requestedEvent.id ? { ...prev, registered: true, registeredCount: prev.registeredCount + 1, spotsLeft: Math.max(0, prev.spotsLeft - 1) } : prev)
        setToast({ message: t(locale, 'going', { name: eventCopy(requestedEvent, locale).title }), tone: 'success' })
      } catch (error) { setToast({ message: error.message, tone: 'error' }) }
    } else {
      setView(modal?.afterLoginView || 'events')
      setToast({ message: t(locale, 'welcome', { name: current.name.split(' ')[0] }), tone: 'success' })
    }
    await loadEvents({ ...eventParams, page: 0 })
    setModal(null)
  }
  async function logout() { await api.logout(); setUser(null); setView('events'); setSelectedEvent(null); await loadEvents({ ...eventParams, page: 0, status: 'ALL' }); setAccountOpen(false); setMobileNavOpen(false); setToast({ message: t(locale, 'signedOut'), tone: 'success' }) }
  function openAdmin() {
    setAccountOpen(false)
    if (isAdmin) { setView('admin'); setSelectedEvent(null); setMobileNavOpen(false); return }
    if (!user) { setModal({ type: 'login', afterLoginView: 'admin' }); setMobileNavOpen(false); return }
    setToast({ message: t(locale, 'adminOnly'), tone: 'error' })
  }

  const isAdmin = user?.role === 'ADMIN'
  const navItems = [{ id: 'events', label: isAdmin ? t(locale, 'events') : t(locale, 'discover') }, ...(user && !isAdmin ? [{ id: 'registrations', label: t(locale, 'registrations') }] : [])]
  return <div className="app-shell">
    <header className="site-header">
      ...
      <div className="header-actions"><button className={`organizer-link ${view === 'admin' ? 'active' : ''}`} onClick={openAdmin} aria-label={t(locale, 'admin')}><span>{t(locale, 'admin')}</span><ChevronDown size={14} /></button><button className="language-toggle" onClick={() => setLocale(locale === 'th' ? 'en' : 'th')} aria-label={locale === 'th' ? 'Switch to English' : 'เปลี่ยนเป็นภาษาไทย'}>{locale === 'th' ? 'ไทย' : 'EN'}</button>
        {user ? <div className="account-wrap"><button className="account-button" onClick={() => setAccountOpen(!accountOpen)} aria-expanded={accountOpen} aria-controls="account-menu"><span className="avatar">{user.name.charAt(0)}</span><span className="account-copy"><strong>{user.name}</strong><small>{isAdmin ? t(locale, 'administrator') : t(locale, 'member')}</small></span><ChevronDown size={16} /></button>{accountOpen && <div className="account-menu" id="account-menu"><span>{user.email}</span><button onClick={logout}><LogOut size={16} /> {t(locale, 'signOut')}</button></div>}</div> : <><button className="button button-ghost button-small" onClick={() => setModal({ type: 'signup' })}>{t(locale, 'signUp')}</button><button className="button button-primary button-small" onClick={() => setModal({ type: 'login' })}>{t(locale, 'signIn')}</button></>}
        <button className="mobile-menu" aria-label={t(locale, 'openNavigation')} aria-expanded={mobileNavOpen} aria-controls="mobile-navigation" onClick={() => setMobileNavOpen(!mobileNavOpen)}>{mobileNavOpen ? <X /> : <Menu />}</button>
      </div>
      {mobileNavOpen && <nav className="mobile-nav" id="mobile-navigation" aria-label={t(locale, 'discover')}>{navItems.map(item => <button key={item.id} className={view === item.id ? 'active' : ''} onClick={() => { setView(item.id); setSelectedEvent(null); setMobileNavOpen(false) }}>{item.label}</button>)}<button className={view === 'admin' ? 'active' : ''} onClick={openAdmin}>{t(locale, 'admin')}</button>{!user && <><button onClick={() => { setModal({ type: 'signup' }); setMobileNavOpen(false) }}>{t(locale, 'signUp')}</button><button onClick={() => { setModal({ type: 'login' }); setMobileNavOpen(false) }}>{t(locale, 'signIn')}</button></>}{user && <button onClick={logout}>{t(locale, 'signOut')}</button>}</nav>}
    </header>
    <main>{loading ? <Loading /> : view === 'detail' && selectedEvent ? <EventDetail event={selectedEvent} loading={detailLoading} onBack={backToEvents} onRegister={handleRegister} locale={locale} /> : view === 'events' ? <Discover page={eventPage} params={eventParams} loading={eventsLoading} user={user} onRegister={handleRegister} onOpen={openEvent} onParamsChange={loadEvents} locale={locale} /> : view === 'registrations' ? <MyRegistrations onCancel={handleCancel} onOpen={openEvent} locale={locale} /> : <AdminDashboard page={eventPage} params={eventParams} loading={eventsLoading} onParamsChange={loadEvents} refresh={loadEvents} onOpen={openEvent} setModal={setModal} setToast={setToast} locale={locale} />}</main>
    <footer><span>{t(locale, 'footer')}</span><span>{t(locale, 'footerBody')}</span></footer>
    {modal?.type === 'login' && <LoginModal modal={modal} setModal={setModal} onAuthenticated={handleAuthenticated} locale={locale} />}
    {modal?.type === 'signup' && <SignupModal modal={modal} setModal={setModal} onAuthenticated={handleAuthenticated} locale={locale} />}
...
function EventCard({ event, featured, onOpen, onRegister, locale }) {
  ...
  return <article className={`event-card ${featured ? 'event-card-featured' : ''}`} tabIndex="0" onClick={() => onOpen(event)} onKeyDown={e => { if (e.key === 'Enter') onOpen(event) }}><div className="event-card-image"><img src={imageFor(event)} alt={copy.title} /><span className={`image-status status-${event.status.toLowerCase()}`}>{event.registered ? t(locale, 'registered') : event.status === 'OPEN' ? t(locale, 'registrationOpen') : event.status === 'FULL' ? t(locale, 'full') : t(locale, 'ended')}</span><button className="image-open" aria-label={t(locale, 'eventDetails')}><ArrowRight size={17} /></button></div><div className="event-card-body"><div className="event-card-date"><span>{formatMonth(event.startsAt, locale)}</span><strong>{new Date(event.startsAt).getDate()}</strong></div><div className="event-card-info"><h3>{copy.title}</h3><p>{copy.description}</p><div className="event-card-meta"><span><Clock3 size={14} />{formatTime(event.startsAt, locale)}</span><span><MapPin size={14} />{copy.location}</span></div><div className="card-footer"><span>{event.spotsLeft} {t(locale, 'spotsLeft')}</span><EventAction event={event} onRegister={onRegister} locale={locale} compact /></div></div></div></article>
}

function EventDetail({ event, loading, onBack, onRegister, locale }) {
  const copy = eventCopy(event, locale)
  return <section className="detail-page container"><button className="back-link" onClick={onBack}><ArrowLeft size={16} /> {t(locale, 'backToEvents')}</button><div className="detail-layout"><div className="detail-visual"><img src={imageFor(event)} alt={copy.title} /><div className="detail-visual-caption"><Status event={event} locale={locale} /><span>{formatMonth(event.startsAt, locale)} {new Date(event.startsAt).getDate()}</span></div></div><div className="detail-copy"><span className="eyebrow">{t(locale, 'eventDetails')}</span><h1>{copy.title}</h1><p className="detail-lede">{copy.description}</p><div className="detail-facts"><div><CalendarDays size={18} /><span><small>{t(locale, 'dateTime')}</small><strong>{formatDateTime(event.startsAt, locale)}</strong></span></div><div><MapPin size={18} /><span><small>{t(locale, 'location')}</small><strong>{copy.location}</strong></span></div><div><Users size={18} /><span className="detail-capacity-copy"><small>{t(locale, 'capacity')}</small><strong>{t(locale, 'registeredOf', { count: event.registeredCount, capacity: event.capacity })}</strong><em>{t(locale, 'spotsRemaining', { count: event.spotsLeft })}</em></span></div></div><div className="detail-actions"><EventAction event={event} onRegister={onRegister} locale={locale} /><button className="button button-ghost" onClick={() => navigator.clipboard?.writeText(window.location.href)}>{t(locale, 'shareEvent')}</button></div>{loading && <div className="detail-refresh"><Loading /></div>}<div className="detail-note"><strong>{t(locale, 'aboutEvent')}</strong><p>{copy.description}</p></div></div></div></section>
}
...
function MyRegistrations({ onCancel, onOpen, locale }) { const [items, setItems] = useState(null); const loadRegistrations = () => api.registrations().then(setItems); useEffect(() => { loadRegistrations() }, []); return <section className="container page-section"><div className="page-heading"><span className="eyebrow">{t(locale, 'yourSchedule')}</span><h1>{t(locale, 'myRegistrations')}</h1><p>{t(locale, 'scheduleBody')}</p></div>{!items ? <Loading /> : items.length === 0 ? <EmptyRegistrations locale={locale} /> : <div className="registration-grid">{items.map(({ event }) => { const copy = eventCopy(event, locale); return <article key={event.id} className="registration-card" tabIndex="0" onClick={() => onOpen(event)} onKeyDown={e => { if (e.key === 'Enter') onOpen(event) }}><img src={imageFor(event)} alt={copy.title} /><div className="registration-card-content"><span className="status status-confirmed"><TicketCheck size={13} /> {t(locale, 'confirmed')}</span><h2>{copy.title}</h2><div className="registration-meta"><p><CalendarDays size={15} /> {formatDateTime(event.startsAt, locale)}</p><p><MapPin size={15} /> {copy.location}</p></div><button className="button button-ghost-danger" onClick={e => { e.stopPropagation(); onCancel(event, loadRegistrations) }}>{t(locale, 'cancel')}</button></div></article> })}</div>}</section> }
...
function SignupModal({ modal, setModal, onAuthenticated, locale }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      await onAuthenticated(current, undefined)
    } catch (err) {
      setError(err.message === 'Something went wrong. Please try again.' ? t(locale, 'accountExists') : err.message); setBusy(false)
    }
  }
  return <div className="overlay centered" role="presentation"><div className="modal signup-modal" role="dialog" aria-modal="true" aria-labelledby="signup-title"><button className="icon-button modal-close" onClick={() => setModal(null)} aria-label={t(locale, 'close')}><X /></button><div className="login-mark signup-mark"><Plus /></div><span className="eyebrow">{t(locale, 'memberAccess')}</span><h2 id="signup-title">{t(locale, 'createAccount')}</h2><p>{t(locale, 'createAccountBody')}</p><form onSubmit={submit}><label>{t(locale, 'displayName')}<input autoFocus required minLength="2" autoComplete="name" value={name} onChange={e => setName(e.target.value)} placeholder={locale === 'th' ? 'เช่น นลิน' : 'e.g. Alex Morgan'} /></label><label>{t(locale, 'email')}<input required type="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" /></label><div className="form-row signup-password-row"><label>{t(locale, 'password')}<input required minLength="8" type="password" autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} placeholder={t(locale, 'passwordHint')} /></label><label>{t(locale, 'confirmPassword')}<input required minLength="8" type="password" autoComplete="new-password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder={t(locale, 'confirmPassword')} /></label></div>{error && <p className="form-error">{error}</p>}<button className="button button-primary full" disabled={busy}>{busy ? t(locale, 'signingUp') : t(locale, 'signUp')}</button></form><p className="auth-switch">{t(locale, 'hasAccount')} <button type="button" onClick={() => setModal({ type: 'login', afterLoginView: modal.afterLoginView })}>{t(locale, 'switchToLogin')}</button></p></div></div>
}
