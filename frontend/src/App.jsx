...
function AdminDashboard({ page, onNavigate, locale }) {
  ...
  const activity = [{ label: locale === 'th' ? 'เข้าชมเว็บ' : 'Visits', value: analytics?.siteViews || 0 }, { label: locale === 'th' ? 'ดูอีเวนต์' : 'Event views', value: analytics?.eventViews || 0 }, { label: locale === 'th' ? 'จองบัตร' : 'Bookings', value: analytics?.registrations || 0 }]
  const maxActivity = Math.max(...activity.map(item => item.value), 1)
  return <section className="container page-section admin-dashboard-page"><div className="dashboard-hero"><div><span className="eyebrow">{locale === 'th' ? 'ภาพรวมผู้จัดงาน' : 'Organizer overview'}</span><h1>{locale === 'th' ? 'สวัสดี, ดูภาพรวมงานของคุณ' : 'A clear view of your events.'}</h1><p>{locale === 'th' ? 'ติดตามความสนใจ การเข้าชม และการจองก่อนลงมือจัดการอีเวนต์' : 'Track attention, visits, and reservations before managing your events.'}</p></div><button className="button button-dark" onClick={() => onNavigate('admin-events')}><Edit3 size={17} /> {locale === 'th' ? 'จัดการอีเวนต์' : 'Manage events'}</button></div><div className="dashboard-metrics">{cards.map(([label, value, note]) => <article key={label}><span>{label}</span><strong>{value ?? '—'}</strong><small>{note}</small></article>)}</div><section className="dashboard-chart"><div><span className="eyebrow">{locale === 'th' ? 'กิจกรรมบนแพลตฟอร์ม' : 'Platform activity'}</span><h2>{locale === 'th' ? 'จากการเข้าชมสู่การจอง' : 'From attention to bookings'}</h2></div><div className="chart-bars">{activity.map(item => <div key={item.label}><span style={{ height: `${Math.max(10, (item.value / maxActivity) * 100)}%` }}><b>{item.value}</b></span><small>{item.label}</small></div>)}</div></section><section className="dashboard-insight"><div><span className="eyebrow">{locale === 'th' ? 'สิ่งที่ควรทำต่อ' : 'Next up'}</span><h2>{locale === 'th' ? 'ดูแลการจองให้ง่ายในทุกจุด' : 'Keep every booking detail in one place.'}</h2><p>{locale === 'th' ? 'เพิ่มอีเวนต์ แก้ไขบัตร ดูรายชื่อผู้ลงทะเบียน และเตรียมเช็กอินได้จากหน้าจัดการอีเวนต์' : 'Create events, edit ticket types, review registrants, and prepare check-in from event management.'}</p></div><button className="button button-outline" onClick={() => onNavigate('admin-events')}>{locale === 'th' ? 'ไปหน้าจัดการ' : 'Open management'} <ArrowRight size={16} /></button></section></section>
}
