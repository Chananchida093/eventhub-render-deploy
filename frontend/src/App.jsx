...
const translations = {
  ...
  th: {
    ...
    ticketFrom: 'บัตรเริ่มต้น ฿{{price}}', ticketTypes: 'ประเภทบัตร', chooseTicket: 'เลือกบัตรของคุณ', ticketQuantity: 'จำนวนบัตร', remaining: 'เหลือ {{count}} ใบ', free: 'ฟรี', orderSummary: 'สรุปรายการ', total: 'รวมทั้งหมด', reserveTicket: 'ยืนยันการจองบัตร', ticketReady: 'e-ticket ของคุณพร้อมแล้ว', ticketCode: 'รหัสบัตร', ticketPolicy: 'ยืนยันบัตรทันที หากยกเลิก ที่นั่งทั้งหมดในรายการนี้จะถูกคืน', addTicketType: 'เพิ่มประเภทบัตร', ticketName: 'ชื่อบัตร', ticketDescription: 'รายละเอียดบัตร', ticketPrice: 'ราคา (บาท)', ticketQuota: 'โควตาบัตร', ticketQuotaNote: 'โควตาบัตรทุกประเภทต้องรวมเท่ากับจำนวนผู้เข้าร่วมสูงสุด', ticketPlanTotal: 'จัดสรรโควตาแล้ว {{count}} จาก {{capacity}} ที่นั่ง', purchaser: 'ผู้จอง', ticketPurchase: 'การจองบัตร', selectedTicket: 'บัตรที่เลือก',
  },
}
...
function App() {
  ...
  useEffect(() => {
    const sessionId = sessionStorage.getItem('gather-visit') || crypto.randomUUID(); sessionStorage.setItem('gather-visit', sessionId)
    api.trackVisit({ type: 'SITE', sessionId }).catch(() => {})
  }, [])
  ...
  async function openEventDetail(event) {
    setModal(null)
    api.trackVisit({ type: 'EVENT', eventId: String(event.id), sessionId: sessionStorage.getItem('gather-visit') || 'anonymous' }).catch(() => {})
    setDetailEvent(await api.event(event.id).catch(() => event))
    setView('detail')
  ...
  return <div className="app-shell">
    <header className="site-header">
      ...
      <div className="header-actions">
        ...
        </div> : <div className="auth-actions"><button className="button button-plain button-small" onClick={() => { setDetailEvent(null); setView('signup') }}>{locale === 'th' ? 'สมัครสมาชิก' : 'Create account'}</button><button className="button button-dark button-small" onClick={() => setModal({ type: 'login' })}>{t(locale, 'signIn')}</button></div>}
      ...
      {menuOpen && <nav className="mobile-nav" aria-label={t(locale, 'discover')}>
        ...
        {!user && <><button onClick={() => { setDetailEvent(null); setView('signup'); setMenuOpen(false) }}>{locale === 'th' ? 'สมัครสมาชิก' : 'Create account'}</button><button onClick={() => { setModal({ type: 'login' }); setMenuOpen(false) }}>{t(locale, 'signIn')}</button></>}
    ...
    <main>
      {loading ? <Loading /> : view === 'detail' && detailEvent ? <EventDetailPage event={detailEvent} onBack={closeEventDetail} onRegister={handleRegister} locale={locale} />
        : view === 'signup' ? <SignupPage setUser={setUser} setView={setView} setModal={setModal} refresh={loadEvents} setToast={setToast} locale={locale} />
...
function DetailImages({ event, placement }) {
  const images = event.detailImages?.length ? event.detailImages : event.detailImageUrl ? [{ url: event.detailImageUrl, placement: 'DETAILS' }] : []
  return <>{images.filter(image => image.placement === placement).map((image, index) => <img className="event-detail-image" key={`${image.url}-${index}`} src={image.url} alt="Event detail" />)}</>
}

function DetailImageEditor({ images, onChange, locale }) {
  const add = () => onChange([...(images || []), { url: '', placement: 'DETAILS' }])
  const change = (index, key, value) => onChange(images.map((image, imageIndex) => imageIndex === index ? { ...image, [key]: value } : image))
  const upload = async (index, file) => { if (!file) return; change(index, 'url', await api.uploadImage(file)) }
  return <section className="detail-image-editor"><div><strong>{locale === 'th' ? 'รูปประกอบในรายละเอียด' : 'Detail content images'}</strong><small>{locale === 'th' ? 'เพิ่มได้หลายรูป อัปโหลดไฟล์ และเลือกตำแหน่งที่ต้องการแสดง' : 'Upload multiple images and choose where each appears.'}</small></div>{images.map((image, index) => <div className="detail-image-row" key={index}><img src={image.url || '/art-design.svg'} alt="" /><label className="upload-button">{image.url ? (locale === 'th' ? 'เปลี่ยนรูป' : 'Replace image') : (locale === 'th' ? 'อัปโหลดรูป' : 'Upload image')}<input type="file" accept="image/*" onChange={e => upload(index, e.target.files?.[0])} /></label><select value={image.placement} onChange={e => change(index, 'placement', e.target.value)}><option value="TOP">{locale === 'th' ? 'ก่อนรายละเอียด' : 'Before details'}</option><option value="DETAILS">{locale === 'th' ? 'ในรายละเอียด' : 'In details'}</option><option value="BOTTOM">{locale === 'th' ? 'ท้ายรายละเอียด' : 'After details'}</option></select><button type="button" onClick={() => onChange(images.filter((_, imageIndex) => imageIndex !== index))} aria-label="Remove image"><Trash2 size={15} /></button></div>)}<button type="button" className="button button-plain button-small" onClick={add}><Plus size={15} /> {locale === 'th' ? 'เพิ่มรูปประกอบ' : 'Add detail image'}</button></section>
}
...
function MyTickets({ locale }) {
  ...
  const tickets = items?.flatMap(({ event, ticketCode, ticketTypeName, quantity, totalPrice, registeredAt }) => Array.from({ length: quantity }, (_, index) => ({ event, ticketCode: quantity > 1 ? `${ticketCode}-${index + 1}` : ticketCode, ticketTypeName, quantity: 1, totalPrice: Number(totalPrice) / quantity, registeredAt, seat: index + 1, total: quantity })))
  return <><section className="container page-section tickets-page"><div className="page-heading"><div className="eyebrow">{locale === 'th' ? 'บัตรพร้อมใช้งาน' : 'Tickets ready'}</div><h1>{locale === 'th' ? 'ตั๋วของฉัน' : 'My tickets'}</h1><p>{locale === 'th' ? 'บัตรทุกใบมี QR ของตัวเอง แตะ QR เพื่อขยายแล้วแสดงให้ทีมงานสแกน' : 'Every ticket has its own QR. Tap it to enlarge before check-in.'}</p></div>{!tickets ? <Loading /> : tickets.length === 0 ? <EmptyRegistrations locale={locale} /> : <div className="ticket-wallet">{tickets.map(({ event, ticketCode, ticketTypeName, totalPrice, registeredAt, seat, total }) => <article className="wallet-ticket" key={ticketCode}><div className="wallet-art"><img src={imageFor(event)} alt="" /><div><span>{categoryLabel(event.category, locale)} · {locale === 'th' ? `ใบที่ ${seat}/${total}` : `Ticket ${seat}/${total}`}</span><h2>{eventCopy(event, locale).title}</h2></div></div><div className="wallet-details"><div><small>{locale === 'th' ? 'วันและเวลา' : 'Date & time'}</small><strong>{formatDate(event.startsAt, locale)} · {formatTime(event.startsAt, locale)}</strong></div><div><small>{locale === 'th' ? 'สถานที่' : 'Location'}</small><strong>{eventCopy(event, locale).location}</strong></div><div><small>{locale === 'th' ? 'ประเภทบัตร' : 'Ticket type'}</small><strong>{ticketTypeName} · {formatPrice(totalPrice, locale)}</strong></div><small>{locale === 'th' ? 'ออกบัตรเมื่อ' : 'Issued'} {formatDate(registeredAt, locale)}</small></div><button className="wallet-scan qr-trigger" onClick={() => setExpandedCode(ticketCode)}><TicketQr code={ticketCode} /><strong>{ticketCode}</strong><span>{locale === 'th' ? 'แตะเพื่อขยาย QR' : 'Tap to enlarge QR'}</span></button></article>)}</div>}</section>{expandedCode && <div className="overlay centered qr-overlay" onClick={() => setExpandedCode(null)}><button className="qr-expanded" onClick={() => setExpandedCode(null)} aria-label={locale === 'th' ? 'ย่อ QR' : 'Close QR'}><TicketQr code={expandedCode} /><strong>{expandedCode}</strong><span>{locale === 'th' ? 'แตะอีกครั้งเพื่อย่อ' : 'Tap again to close'}</span></button></div>}</>
}
...
function StaffDesk({ locale }) {
  ...
  useEffect(() => { api.events({ size: 100, status: 'OPEN' }).then(data => setEvents(data.items)) }, [])
  async function chooseEvent(event) { setSelectedEvent(event); setResult(null); setError(''); const [all, latest] = await Promise.all([api.staffAttendees(event.id), api.recentCheckIns(event.id)]); setAttendees(all); setRecent(latest) }
  async function submit(e) { e.preventDefault(); if (!code.trim() || !selectedEvent) return; setBusy(true); setError(''); try { const checked = await api.checkIn(selectedEvent.id, code); setResult(checked); setCode(''); const [all, latest] = await Promise.all([api.staffAttendees(selectedEvent.id), api.recentCheckIns(selectedEvent.id)]); setAttendees(all); setRecent(latest) } catch (err) { setResult(null); setError(err.message) } finally { setBusy(false) } }
  if (!selectedEvent) return <section className="container page-section staff-page"><div className="page-heading"><div className="eyebrow"><ScanLine size={14} /> {locale === 'th' ? 'ทีมหน้างาน' : 'Event staff'}</div><h1>{locale === 'th' ? 'เลือกอีเวนต์ที่จะเช็กอิน' : 'Choose an event to check in'}</h1><p>{locale === 'th' ? 'เลือกงานก่อนเริ่มสแกน เพื่อป้องกันการใช้ตั๋วผิดงาน' : 'Pick the event first to keep check-in accurate.'}</p></div>{!events ? <Loading /> : <div className="staff-event-list">{events.map(event => <button key={event.id} onClick={() => chooseEvent(event)}><img src={imageFor(event)} alt="" /><span><strong>{eventCopy(event, locale).title}</strong><small>{formatDate(event.startsAt, locale)} · {eventCopy(event, locale).location}</small></span><ArrowRight size={18} /></button>)}</div>}</section>
  return <section className="container page-section staff-page"><button className="back-link" onClick={() => setSelectedEvent(null)}><ArrowLeft size={16} /> {locale === 'th' ? 'เปลี่ยนอีเวนต์' : 'Change event'}</button><div className="page-heading compact"><div className="eyebrow"><ScanLine size={14} /> {locale === 'th' ? 'ทีมหน้างาน' : 'Event staff'}</div><h1>{locale === 'th' ? 'เช็กอินผู้เข้าร่วม' : 'Check in attendees'}</h1><p>{eventCopy(selectedEvent, locale).title} · {attendees.length} {locale === 'th' ? 'ผู้ลงทะเบียน' : 'registrants'}</p></div><div className="staff-checkin"><form onSubmit={submit}><label>{locale === 'th' ? 'รหัสตั๋ว' : 'Ticket code'}<input autoFocus value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="GTH-XXXXXXXXXX" /></label><button className="button button-dark" disabled={busy}><ScanLine size={17} /> {busy ? (locale === 'th' ? 'กำลังตรวจสอบ…' : 'Checking…') : (locale === 'th' ? 'เช็กอินตั๋ว' : 'Check in ticket')}</button>{error && <p className="form-error">{error}</p>}</form>{result && <div className="checkin-result"><ShieldCheck size={28} /><span>{locale === 'th' ? 'เช็กอินสำเร็จ' : 'Check-in successful'}</span><h2>{result.attendeeName}</h2><p>{result.eventTitle}</p><strong>{result.ticketType} × {result.quantity}</strong><small>{result.ticketCode} · {formatTime(result.checkedInAt, locale)}</small></div>}</div><section className="staff-roster"><div><span className="eyebrow">{locale === 'th' ? 'เช็กอินล่าสุด' : 'Latest check-ins'}</span><h2>{locale === 'th' ? 'คนที่เพิ่งมาถึง' : 'Just arrived'}</h2>{recent.length ? recent.map(person => <p key={person.ticketCode}><strong>{person.name}</strong><span>{person.ticketType} · {formatTime(person.checkedInAt, locale)}</span></p>) : <small>{locale === 'th' ? 'ยังไม่มีการเช็กอิน' : 'No check-ins yet'}</small>}</div><div><span className="eyebrow">{locale === 'th' ? 'ผู้ลงทะเบียนทั้งหมด' : 'All registrants'}</span><h2>{attendees.length} {locale === 'th' ? 'ราย' : 'people'}</h2>{attendees.map(person => <p key={person.ticketCode}><strong>{person.name}</strong><span>{person.email} · {person.checkedInAt ? (locale === 'th' ? 'เช็กอินแล้ว' : 'Checked in') : (locale === 'th' ? 'รอเช็กอิน' : 'Pending')}</span></p>)}</div></section></section>
}
...
function PurchaseModal({ event, onClose, refresh, setToast, locale }) {
  ...
  async function purchase() {
    ...
    try { const booked = await api.register(event.id, { ticketTypeId: selected.id, quantity: safeQuantity }); setReceipt(booked); await refresh(); setStep('success'); setToast({ message: t(locale, 'ticketReady'), tone: 'success' }) }
    catch (err) { setError(err.message); setBusy(false) }
  }
  const copy = eventCopy(event, locale)
  const paymentCode = `GATHER-PAY-${event.id}-${selected?.id || 'TICKET'}-${safeQuantity}`
  return <div className="overlay centered" role="presentation"><div className="modal purchase-modal" role="dialog" aria-modal="true" aria-labelledby="purchase-title">
    ...
    {step === 'choose' && <><div className="ticket-picker"><strong>{t(locale, 'chooseTicket')}</strong>{available.map(ticket => <button key={ticket.id} className={`ticket-option ${selected?.id === ticket.id ? 'selected' : ''}`} onClick={() => { setTicketId(ticket.id); setQuantity(1) }}><span><b>{ticket.name}</b><small>{ticket.description || t(locale, 'selectedTicket')} · {t(locale, 'remaining', { count: ticket.remaining })}</small></span><strong>{formatPrice(ticket.price, locale)}</strong></button>)}</div>{!selected ? <p className="form-error">{t(locale, 'eventFull')}</p> : <><div className="quantity-row"><span><strong>{t(locale, 'ticketQuantity')}</strong><small>{t(locale, 'remaining', { count: maxQuantity })}</small></span><div className="quantity-control"><button disabled={safeQuantity <= 1} onClick={() => setQuantity(safeQuantity - 1)} aria-label="Decrease quantity"><Minus size={15} /></button><strong>{safeQuantity}</strong><button disabled={safeQuantity >= maxQuantity} onClick={() => setQuantity(safeQuantity + 1)} aria-label="Increase quantity"><Plus size={15} /></button></div></div><div className="purchase-total"><span>{t(locale, 'total')}</span><strong>{formatPrice(Number(selected.price) * safeQuantity, locale)}</strong></div></>}<button className="button button-dark full" disabled={!selected} onClick={() => Number(selected?.price) === 0 ? purchase() : setStep('payment')}>{Number(selected?.price) === 0 ? (locale === 'th' ? 'ยืนยันการรับบัตรฟรี' : 'Claim free ticket') : (locale === 'th' ? 'ไปที่การชำระเงิน' : 'Continue to payment')} <ArrowRight size={17} /></button></>}
    {step === 'payment' && <div className="payment-step"><div className="purchase-summary"><span>{selected?.name} × {safeQuantity}</span><strong>{formatPrice(Number(selected?.price) * safeQuantity, locale)}</strong></div><TicketQr code={paymentCode} /><strong>{locale === 'th' ? 'สแกนเพื่อชำระเงิน (ตัวอย่าง)' : 'Scan to pay (mockup)'}</strong><p>{locale === 'th' ? 'QR นี้เป็น mockup เท่านั้น ยังไม่มีการเชื่อมต่อผู้ให้บริการชำระเงิน' : 'This QR is a mockup only; no payment provider is connected.'}</p>{error && <p className="form-error">{error}</p>}<button className="button button-dark full" disabled={busy} onClick={purchase}>{busy ? t(locale, 'working') : (locale === 'th' ? 'จำลองว่าชำระเงินแล้ว' : 'Simulate payment complete')} <Check size={17} /></button><button className="button button-plain full" onClick={() => setStep('choose')}>{locale === 'th' ? 'ย้อนกลับ' : 'Back'}</button></div>}
    {step === 'success' && <div className="payment-success"><ShieldCheck size={36} /><h3>{locale === 'th' ? 'จองสำเร็จแล้ว' : 'Booking confirmed'}</h3><p>{locale === 'th' ? 'ตั๋วของคุณอยู่ในหน้า “ตั๋วของฉัน” พร้อม QR สำหรับเช็กอิน' : 'Your ticket is ready in My tickets with its QR for check-in.'}</p><div className="purchase-summary"><span>{receipt?.ticketTypeName} × {receipt?.quantity}</span><strong>{receipt?.ticketCode}</strong></div><button className="button button-dark full" onClick={onClose}>{locale === 'th' ? 'ปิดหน้านี้' : 'Close'}</button></div>}
    {step === 'choose' && <p className="purchase-policy">{t(locale, 'ticketPolicy')}</p>}
  </div></div>
}
...
function AdminDashboard({ page, onNavigate, locale }) {
  ...
  const number = value => new Intl.NumberFormat(locale === 'th' ? 'th-TH' : 'en-US').format(value || 0)
  const siteViews = analytics?.siteViews ?? 0
  const eventViews = analytics?.eventViews ?? 0
  const registrations = analytics?.registrations ?? page.totalRegistrations ?? 0
  const visibleEvents = page.items || []
  const usedSeats = visibleEvents.reduce((total, event) => total + event.registeredCount, 0)
  const totalSeats = visibleEvents.reduce((total, event) => total + event.capacity, 0)
  const capacityRate = totalSeats > 0 ? Math.round((usedSeats / totalSeats) * 100) : 0
  const funnelData = [
    { name: locale === 'th' ? 'เข้าชมเว็บ' : 'Visits', value: siteViews },
    { name: locale === 'th' ? 'ดูรายละเอียด' : 'Event views', value: eventViews },
    { name: locale === 'th' ? 'จองบัตร' : 'Bookings', value: registrations },
  ]
  const capacityData = [{ name: locale === 'th' ? 'จองแล้ว' : 'Reserved', value: usedSeats }, { name: locale === 'th' ? 'ที่ว่าง' : 'Available', value: Math.max(totalSeats - usedSeats, 0) }]
  const highlight = [
    [locale === 'th' ? 'การจองทั้งหมด' : 'Total bookings', registrations, locale === 'th' ? 'ตั๋วที่ถูกจองในระบบ' : 'Tickets reserved in the platform'],
    ...
    [locale === 'th' ? 'ผู้เข้าชมไม่ซ้ำ' : 'Unique visitors', analytics?.uniqueVisitors ?? 0, locale === 'th' ? 'จากข้อมูลการเข้าชมที่บันทึกไว้' : 'From recorded visits'],
  ]
  return <section className="container page-section admin-dashboard-page">
    <header className="dashboard-hero"><div><span className="eyebrow">{locale === 'th' ? 'ภาพรวมผู้จัดงาน' : 'Organizer overview'}</span><h1>{locale === 'th' ? 'ภาพรวมที่ช่วยให้งานของคุณเดินต่อ' : 'The signals behind your next event.'}</h1><p>{locale === 'th' ? 'ดูการเข้าชม การสนใจ และการจองจากข้อมูลจริงของ Gather แล้วเลือกสิ่งที่ควรจัดการต่อ' : 'Read real Gather visits, event interest, and bookings to decide what needs attention next.'}</p></div><button className="button button-dark" onClick={() => onNavigate('admin-events')}><Edit3 size={17} /> {locale === 'th' ? 'จัดการอีเวนต์' : 'Manage events'}</button></header>
    <section className="dashboard-highlights" aria-label={locale === 'th' ? 'สรุปข้อมูลสำคัญ' : 'Key metrics'}>{highlight.map(([label, value, note]) => <article key={label}><span>{label}</span><strong>{number(value)}</strong><small>{note}</small></article>)}</section>
    <div className="dashboard-data-grid">
      <section className="dashboard-panel dashboard-funnel"><div className="dashboard-panel-heading"><div><span className="eyebrow">{locale === 'th' ? 'เส้นทางการจอง' : 'Booking journey'}</span><h2>{locale === 'th' ? 'จากการเข้าชมสู่การจอง' : 'From a visit to a booking'}</h2></div><p>{analytics ? (locale === 'th' ? 'ข้อมูลที่บันทึกในระบบ' : 'Recorded platform activity') : (locale === 'th' ? 'กำลังรอข้อมูลการเข้าชม' : 'Waiting for visit data')}</p></div><div className="dashboard-chart-canvas"><ResponsiveContainer width="100%" height="100%"><BarChart data={funnelData} margin={{ top: 16, right: 4, left: -22, bottom: 0 }}><XAxis dataKey="name" tickLine={false} axisLine={false} tick={{ fill: '#667085', fontSize: 11 }} /><YAxis allowDecimals={false} tickLine={false} axisLine={false} tick={{ fill: '#98a2b3', fontSize: 10 }} /><Tooltip cursor={{ fill: 'rgba(31, 78, 74, .06)' }} contentStyle={{ border: '1px solid #d9dedb', borderRadius: 8, boxShadow: 'none', fontSize: 12 }} formatter={value => [number(value), locale === 'th' ? 'จำนวนครั้ง' : 'Count']} /><Bar dataKey="value" fill="#1f6f69" radius={[7, 7, 2, 2]} maxBarSize={66} /></BarChart></ResponsiveContainer></div></section>
      <section className="dashboard-panel dashboard-conversion"><div className="dashboard-panel-heading"><div><span className="eyebrow">{locale === 'th' ? 'การใช้ความจุ' : 'Capacity'}</span><h2>{locale === 'th' ? 'ที่นั่งในอีเวนต์' : 'Seats in your events'}</h2></div><span className="dashboard-page-note">{locale === 'th' ? 'อีเวนต์ในหน้านี้' : 'Events on this page'}</span></div><div className="capacity-visual"><div className="capacity-donut"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={capacityData} dataKey="value" innerRadius="69%" outerRadius="92%" startAngle={90} endAngle={-270} stroke="none">{capacityData.map((entry, index) => <Cell key={entry.name} fill={index === 0 ? '#ed765e' : '#edf0ee'} />)}</Pie></PieChart></ResponsiveContainer><div><strong>{capacityRate}%</strong><span>{locale === 'th' ? 'ถูกจองแล้ว' : 'reserved'}</span></div></div><div className="capacity-copy"><strong>{number(usedSeats)} <span>/ {number(totalSeats)}</span></strong><p>{locale === 'th' ? 'ที่นั่งถูกจองจากรายการอีเวนต์ที่กำลังแสดง' : 'seats reserved from the events currently listed'}</p><div><i className="legend-used" />{locale === 'th' ? 'จองแล้ว' : 'Reserved'}<i className="legend-open" />{locale === 'th' ? 'ที่ว่าง' : 'Available'}</div></div></div></section>
    </div>
    <section className="dashboard-operations"><div className="dashboard-operations-heading"><div><span className="eyebrow">{locale === 'th' ? 'อีเวนต์ที่ต้องดูแล' : 'Events to watch'}</span><h2>{locale === 'th' ? 'ดูความจุของอีเวนต์ที่กำลังเปิดอยู่' : 'Keep an eye on active event capacity'}</h2></div><button className="button button-outline" onClick={() => onNavigate('admin-events')}>{locale === 'th' ? 'เปิดหน้าจัดการ' : 'Open management'} <ArrowRight size={16} /></button></div><div className="capacity-list">{visibleEvents.slice(0, 4).map(event => { const fill = event.capacity ? Math.round((event.registeredCount / event.capacity) * 100) : 0; return <article key={event.id}><div><strong>{eventCopy(event, locale).title}</strong><span>{formatDate(event.startsAt, locale)} · {eventCopy(event, locale).location}</span></div><div className="event-fill"><span><i style={{ width: `${fill}%` }} /></span><strong>{number(event.registeredCount)} / {number(event.capacity)}</strong></div></article> })}{visibleEvents.length === 0 && <p className="dashboard-empty">{locale === 'th' ? 'ยังไม่มีอีเวนต์สำหรับแสดงในแดชบอร์ด' : 'There are no events to show in this dashboard yet.'}</p>}</div></section>
  </section>
}
...
function EventEditor({ event, onClose, onSaved, locale }) {
  const initial = event ? { ...event, imageUrl: event.imageUrl || '', detailImages: event.detailImages || (event.detailImageUrl ? [{ url: event.detailImageUrl, placement: 'DETAILS' }] : []), startsAt: event.startsAt.slice(0,16), ticketTypes: ticketTypesFor(event).map(ticket => ({ ...ticket, price: String(ticket.price) })) } : { title:'', description:'', location:'', startsAt:'', capacity:30, category:'COMMUNITY', imageUrl:'/art-design.svg', detailImages:[], ticketTypes:[{ name:'General admission', description:'', price:'0', capacity:30 }] }
  ...
  return <div className="overlay" role="presentation"><div className="sheet" role="dialog" aria-modal="true" aria-labelledby="event-form-title">
    ...
    <form onSubmit={submit} className="event-form"><label>{t(locale, 'eventTitle')}<input required autoFocus {...field('title')} placeholder={t(locale, 'temporaryTitle')} /></label><label>{t(locale, 'description')}<textarea required rows="5" {...field('description')} placeholder={t(locale, 'descriptionHint')} /></label><section className="cover-editor"><div><strong><ImageIcon size={16} /> {locale === 'th' ? 'รูปปกอีเวนต์' : 'Event cover image'}</strong><small>{locale === 'th' ? 'เลือกภาพในระบบหรือวาง URL ของรูปภาพ' : 'Choose a built-in visual or paste an image URL.'}</small></div><div className="cover-options">{['/art-design.svg','/art-spring.svg','/art-product.svg','/art-accessibility.svg'].map(src => <button type="button" key={src} className={form.imageUrl === src ? 'active' : ''} onClick={() => setForm({ ...form, imageUrl: src })}><img src={src} alt="" /></button>)}{form.imageUrl && !form.imageUrl.startsWith('/art-') && <button type="button" className="active cover-url-preview" aria-label="Cover URL preview"><img src={form.imageUrl} alt="Cover preview" /></button>}</div><label className="upload-button">{locale === 'th' ? 'อัปโหลดรูปปก' : 'Upload cover image'}<input type="file" accept="image/*" onChange={async e => { const file = e.target.files?.[0]; if (file) setForm({ ...form, imageUrl: await api.uploadImage(file) }) }} /></label></section><DetailImageEditor images={form.detailImages} onChange={detailImages => setForm({ ...form, detailImages })} locale={locale} /><div className="form-row"><label>{t(locale, 'location')}<input required {...field('location')} placeholder={t(locale, 'locationHint')} /></label><label>{t(locale, 'category')}<select required {...field('category')}>{['TECH','DESIGN','CAREER','COMMUNITY'].map(category => <option key={category} value={category}>{categoryLabel(category, locale)}</option>)}</select></label></div><div className="form-row"><label>{t(locale, 'dateTime')}<input required type="datetime-local" {...field('startsAt')} /></label><label>{t(locale, 'maximum')}<input required type="number" min={event?.registeredCount || 1} {...field('capacity')} /></label></div><div className="ticket-editor"><div className="ticket-editor-heading"><div><strong>{t(locale, 'ticketTypes')}</strong><small>{t(locale, 'ticketQuotaNote')}</small></div><button type="button" className="button button-plain button-small" onClick={() => setForm({ ...form, ticketTypes:[...form.ticketTypes, { name:'', description:'', price:'0', capacity:1 }] })}><Plus size={15} /> {t(locale, 'addTicketType')}</button></div>{form.ticketTypes.map((ticket, index) => <div className="ticket-edit-row" key={ticket.id ?? index}><div className="ticket-edit-top"><strong>0{index + 1}</strong>{form.ticketTypes.length > 1 && <button type="button" onClick={() => setForm({ ...form, ticketTypes: form.ticketTypes.filter((_, ticketIndex) => ticketIndex !== index) })} aria-label="Remove ticket type"><Trash2 size={15} /></button>}</div><label>{t(locale, 'ticketName')}<input required value={ticket.name} onChange={e => changeTicket(index, 'name', e.target.value)} /></label><label>{t(locale, 'ticketDescription')}<input value={ticket.description || ''} onChange={e => changeTicket(index, 'description', e.target.value)} /></label><div className="form-row"><label>{t(locale, 'ticketPrice')}<input required min="0" step="1" type="number" value={ticket.price} onChange={e => changeTicket(index, 'price', e.target.value)} /></label><label>{t(locale, 'ticketQuota')}<input required min="1" type="number" value={ticket.capacity} onChange={e => changeTicket(index, 'capacity', e.target.value)} /></label></div></div>)}</div><p className={`ticket-allocation ${ticketTotal === Number(form.capacity) ? 'valid' : ''}`}>{t(locale, 'ticketPlanTotal', { count: ticketTotal, capacity: form.capacity })}</p>{error && <p className="form-error">{error}</p>}<div className="form-actions"><button type="button" className="button button-plain" onClick={onClose}>{t(locale, 'cancelAction')}</button><button className="button button-dark" disabled={saving || ticketTotal !== Number(form.capacity)}>{saving ? t(locale, 'saving') : event ? t(locale, 'saveChanges') : t(locale, 'createEvent')}</button></div></form>
  </div></div>
}
...
function SignupPage({ setUser, setView, setModal, refresh, setToast, locale }) {
  ...
  async function submit(e) {
    e.preventDefault()
    if (password !== confirmPassword) { setError(locale === 'th' ? 'รหัสผ่านทั้งสองช่องไม่ตรงกัน' : 'Passwords do not match'); return }
    setBusy(true); setError('')
    try {
      const current = await api.signup({ name, email, password })
      setUser(current); await refresh(); setView('events')
      setToast({ message: locale === 'th' ? `ยินดีต้อนรับ, ${current.name.split(' ')[0]}` : `Welcome, ${current.name.split(' ')[0]}`, tone: 'success' })
    } catch (err) { setError(err.message); setBusy(false) }
  }
  return <section className="signup-page"><div className="signup-intro"><div><span className="eyebrow">{locale === 'th' ? 'เริ่มต้นกับ Gather' : 'Start with Gather'}</span><h1>{locale === 'th' ? 'ทุกอีเวนต์ที่อยากไป เริ่มจากบัญชีเดียว' : 'One account for every event you want to attend.'}</h1><p>{locale === 'th' ? 'เก็บบัตร รายการจอง และข้อมูลเช็กอินของคุณไว้พร้อมใช้ทุกครั้งที่มีกิจกรรม' : 'Keep your tickets, bookings, and check-in details ready for every event.'}</p></div><ul><li><Check size={17} /> {locale === 'th' ? 'จองและเก็บบัตรของคุณในที่เดียว' : 'Reserve and keep every ticket in one place'}</li><li><Check size={17} /> {locale === 'th' ? 'เปิด QR เพื่อเช็กอินได้ทันที' : 'Open your QR code when it is time to check in'}</li><li><Check size={17} /> {locale === 'th' ? 'จัดการรายการจองได้ทุกเมื่อ' : 'Manage bookings whenever plans change'}</li></ul></div><div className="signup-card"><div className="login-mark"><TicketCheck /></div><span className="eyebrow">{locale === 'th' ? 'สมัครสมาชิก' : 'Create account'}</span><h2>{locale === 'th' ? 'สร้างบัญชีของคุณ' : 'Create your account'}</h2><p>{locale === 'th' ? 'ใช้เวลาเพียงครู่เดียว แล้วเริ่มจองอีเวนต์ได้เลย' : 'It takes a moment, then you are ready to reserve.'}</p><form onSubmit={submit}><label>{locale === 'th' ? 'ชื่อที่แสดง' : 'Display name'}<input autoFocus required minLength="2" value={name} onChange={e => setName(e.target.value)} placeholder={locale === 'th' ? 'เช่น มาลี ศรีสุข' : 'e.g. Mali Srisuk'} /></label><label>{t(locale, 'email')}<input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" /></label><label>{locale === 'th' ? 'รหัสผ่าน' : 'Password'}<input required type="password" minLength="8" value={password} onChange={e => setPassword(e.target.value)} placeholder={locale === 'th' ? 'อย่างน้อย 8 ตัวอักษร' : 'At least 8 characters'} /></label><label>{locale === 'th' ? 'ยืนยันรหัสผ่าน' : 'Confirm password'}<input required type="password" minLength="8" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} /></label>{error && <p className="form-error">{error}</p>}<button className="button button-dark full" disabled={busy}>{busy ? (locale === 'th' ? 'กำลังสร้างบัญชี…' : 'Creating account…') : (locale === 'th' ? 'สร้างบัญชีและเริ่มจอง' : 'Create account')}</button></form><p className="signup-switch">{locale === 'th' ? 'มีบัญชีอยู่แล้ว?' : 'Already have an account?'} <button onClick={() => { setView('events'); setModal({ type: 'login' }) }}>{t(locale, 'signIn')}</button></p></div></section>
}
